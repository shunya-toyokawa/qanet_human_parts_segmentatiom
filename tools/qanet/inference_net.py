...
from qanet.core.inference import Inference
...
def inference(cfg,inference_engine, loader, datasets, all_hooks):
    print("in inference fanction")
    #print(cfg,inference_engine, loader, datasets, all_hooks)
    print(cfg)
    total_timer = Timer()
    total_timer.tic()
    eval = Evaluation(cfg)
    ...
    with torch.no_grad():
        ...
        #for i in range(len(loader)):
            ...
            all_hooks.data_toc()
            all_hooks.infer_tic()
            #print("ppppppppppppppppp",input,targets)

            result = inference_engine(inputs, targets)
            print(result)
            all_hooks.infer_toc()
            all_hooks.post_tic()
...
def main(args):
    ...
    inference_engine = Inference(cfg,model)
    #print(inference_engine)

    # Test
    #test(cfg, test_engine, test_loader, datasets, all_hooks)
    inference(cfg, test_engine, test_loader, datasets, all_hooks)
